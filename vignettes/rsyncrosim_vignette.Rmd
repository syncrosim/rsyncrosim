---
title: "Introduction to `rsyncrosim` v1"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to `rsyncrosim` v1}
...
```{r delete SyncroSim packages, echo = FALSE, results = FALSE, message = FALSE, eval = FALSE}
...
Currently we do not have the `helloworldEnhanced` package installed! Install `helloworldEnhanced` using the `rynscrosim` function `addPackage()`. This function takes a package name as input and then queries the SyncroSim package server for the specified package.
...
```{r check version, warning = FALSE, eval = FALSE}
version(mySession)
```
...
```{r view specific datasheet, warning = FALSE, eval = FALSE}
...
```{r assign input data, warning = FALSE, eval = FALSE}
...
```{r check input data, warning = FALSE, eval = FALSE}
