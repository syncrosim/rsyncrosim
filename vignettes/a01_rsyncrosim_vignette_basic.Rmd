...
From this output we can see the the `Run Control` datasheet and `Inputs` only accept a single row of data (i.e. `isSingle = TRUE`). This is something to consider when we configure our model inputs.
...
Currently our input scenario datasheets are empty! To run our scenario we will need to add some values to the Pipeline (`core_Pipeline`) and Input Datasheet (`InputDatasheet`) so we can run our model. 

First, we will add data to the Pipeline datasheet. The Pipeline datasheet determines which transformers the scenarios will run and in which order. Use the code below to assign the pipeline datasheet to a new data frame variable.
...
```{r check pipeline data, warning = FALSE}
str(myPipeline)
```

The Pipeline data frame requires 2 values:

* `StageNameId` : the ID of the pipeline stage (transformer). This column is a factor that has only a single level: "Hello World Time (R)". 
* `RunOrder` : the numerical order in which the stages will be run.

Now, we will update the pipeline data frame. This can be done in many ways (e.g. using the `dplyr` package), but `rsyncrosim` also provides a helper function called `addRow()` for easily adding new rows to R data frames. The `addRow()` function takes the `targetDataframe` as the first value (in this case, our Pipeline data frame that we want to update), and the data frame of new rows to append to the input data frame as the second value.
...
Next, assign the Inputs datasheet to a new data frame variable.
...
Check the columns that need input values and the type of values these columns require (e.g. string, numeric, logical) using the `str()` base R function.
...
Now we will update the input data frame using the `addRow()` function. Note that in the previous section we discovered that the Inputs datasheet only accepts a single row of values, so we can only have one value each for our slope (*m*) and intercept (*b*).
...
Now that we have complete data frames of the Pipeline and Inputs, we will save these data frames to their respective SyncroSim datasheets using the `saveDatasheet()` function. Since these datasheets are both scenario-scoped, we will save both of them at the scenario level by setting `ssimObject = myScenario`.
