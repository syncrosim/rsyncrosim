...
Currently our input scenario datasheets are empty! We need to add some values to our pipeline ('core_Pipeline'), and input datasheets (`InputDatasheet`) so we can run our model. First, we assign the pipeline datasheet to a new data frame variable.
...
Check the columns that need input values and the type of values these columns require (e.g. string, numeric, logical) using the `srt()` base R function.

```{r check input data, warning = FALSE}
str(myPipeline)
```

The pipeline dataframe requiress 2 values:

* `StageNameId` : the ID of the pipeline stage.
* `RunOrder` : the numerical order in which the stages will be run.

Now, we will update the pipeline dataframe. This can be done in many ways (e.g. using the `dplyr` package), but `rsyncrosim` also provides a helper function called `addRow()` for easily adding new rows to R data frames. The `addRow()` function takes the `targetDataframe` as the first value (in this case, our pipeline data frame that we want to update), and the data frame of new rows to append to the input data frame as the second value.

```{r add input data not run, warning = FALSE}
# Create pipeline data and add it to the pieline data frame
...
Next, assign the input datasheet to a new data frame variable.
...
Now we will update the input data frame using the `addRow()` function. Note that in the previous section we discovered that the input datasheets only accept a single row of values, so we can only have one value each for our slope (*m*) and intercept (*b*).
...
Now that we have complete data frames of pipeline and input values, we will save these data frames to their respective SyncroSim datasheet using the `saveDatasheet()` function. We can save our datasheets at the library, project, or scenario level, but in this case we will save both of them at the scenario level.
