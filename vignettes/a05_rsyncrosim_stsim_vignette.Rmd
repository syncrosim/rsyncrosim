...
```{r check version not run, warning = FALSE}
version(mySession)
```
...
```{r add package from server not run, warning=FALSE}
# Install stsim
addPackage("stsim")
```
...
```{r load datasheet not run, warning = FALSE}
...
```{r save terminology not run, warning = FALSE}
...
```{r save stratum not run, warning = FALSE}
...
```{r save StateLabelX not run, warning = FALSE}
...
```{r save StateLabelY not run, warning = FALSE}
...
```{r save State Classes not run, warning = FALSE}
...
```{r save transitionTypes not run, warning = FALSE}
...
```{r save transitionGroups not run, warning = FALSE}
...
```{r save transitionTypesGroups not run, warning = FALSE}
...
```{r save ages not run, warning = FALSE}
...
```{r save runControl not run, warning = FALSE}
...
```{r save dTransitions run, warning = FALSE}
...
```{r save pTransitions run, warning = FALSE}
...
```{r save ICSpatial no run, warning = FALSE}
...
```{r check initial conditions rasters, fig.align="center", fig.dim = c(5,5), warning=FALSE}
...
```{r save ICNonSpatial not run, warning = FALSE}
...
```{r save ICNonSpatialDistribution not run, warning = FALSE}
...
```{r save transitionTarget not run, warning = FALSE}
...
```{r save outputOptionsSpatial not run, warning = FALSE}
...
```{r save harvest transitionTarget run, warning = FALSE}
...
```{r view transitionTarget datasheets not run, warning = FALSE}
...
```{r run scenarios not run, warning = FALSE}
# Run both Scenarios
...
```{r view myRastersTimestep5 run, warning = FALSE, fig.align = TRUE, fig.dim = c(5,5)}
