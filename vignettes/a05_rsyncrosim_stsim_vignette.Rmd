...
# Add values as a data frame to an stsim Datasheet
...
```{r}
# Create a new R data frame containing the names of the State Classes and their corresponding StateLabelX ID, StateLabelY ID, and a unique numeric ID
...
```{r}
# Create an R data frame containing a column of transition type names and a column for their unique numerical IDs 
...
* ***Option 1 - Spatial***. Let's first take a look at our rasters.

First we will retrieve the sample raster files (in GeoTIFF format) provided with the `rsyncrosim` package. 
