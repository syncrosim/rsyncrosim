...
```{r add package from server not run, warning = FALSE, eval = FALSE}
# Install stsim
addPackage("stsim")
```

```{r add package from server run, warning = FALSE, echo = FALSE}
addPackage("stsim")
```
...
```{r check version not run, warning = FALSE, eval = FALSE}
version(mySession)
```

```{r check version run, warning = FALSE, echo = FALSE}
version(mySession)
```
...
Currently our input Datasheets are empty! We need to add some values to these Datasheets so we can run our models. We will use the `datasheet()` function to retrieve input Datasheets from our Library, and the `addRows()` function to add rows of values to these Datasheets. Both of these functions are contained in the `rsyncrosim` package. 
...
The Project-scoped Datasheet called `Terminology` specifies terms used across all Scenarios in the same Project. Use the `datasheet()` function to retrieve the current values for this Datasheet, as currently stored in the SyncroSim Library file.

```{r load datasheet not run, warning = FALSE, eval = FALSE}
...
```{r load datasheet run, warning = FALSE, echo = FALSE}
# Load the Terminology Datasheet to a new R data frame 
terminology <- datasheet(myProject, name="stsim_Terminology")
...
```{r save terminology not run, warning = FALSE, eval = FALSE}
...
```{r save terminology run, warning = FALSE, echo = FALSE}
# Edit the values of the StateLabelX and AmountUnits columns
terminology$AmountUnits <- "Hectares"
terminology$StateLabelX <- "Forest Type"
...
```{r save stratum not run, warning = FALSE, eval = FALSE}
...
```{r save stratum run, warning = FALSE, echo = FALSE}
# Load a copy of the Stratum Datasheet.
# To load an empty copy of this Datasheet, specify the argument empty = TRUE in the datasheet() function.
stratum <- datasheet(myProject, "stsim_Stratum", empty = TRUE)
...
```{r save StateLabelX not run, warning = FALSE, eval = FALSE}
...
```{r save StateLabelX run, warning = FALSE, echo = FALSE}
# Create a vector containing the State Class labels 
forestTypes <- c("Coniferous", "Deciduous", "Mixed")
...
```{r save StateLabelY not run, warning = FALSE, eval = FALSE}
...
```{r save StateLabelY run, warning = FALSE, echo = FALSE}
# Add values as a data frame directly to an stsim Datasheet
saveDatasheet(myProject, data.frame(Name = c("All")), "stsim_StateLabelY", force = TRUE)
```

**State Classes**: Combine `StateLabelX` and `StateLabelY`, and assign each class a unique name and ID
```{r save State Classes not run, warning = FALSE, eval = FALSE}
...
```{r save State Classes run, warning = FALSE, echo = FALSE}
# Create a new R data frame that combines the StateLabelX and Y data
stateClasses <- data.frame(Name = forestTypes)
stateClasses$StateLabelXID <- stateClasses$Name
stateClasses$StateLabelYID <- "All"
stateClasses$ID <- c(1, 2, 3)
...
```{r save transitionTypes not run, warning = FALSE, eval = FALSE}
...
```{r save transitionTypes run, warning = FALSE, echo = FALSE}
# Create an R data frame containing transition type data 
transitionTypes <- data.frame(Name = c("Fire", "Harvest", "Succession"), ID = c(1, 2, 3))
...
```{r save transitionGroups not run, warning = FALSE, eval = FALSE}
...
```{r save transitionGroups run, warning = FALSE, echo = FALSE}
# Create an R data frame containing a column of transition type names 
transitionGroups <- data.frame(Name = c("Fire", "Harvest", "Succession"))
...
```{r save transitionTypesGroups not run, warning = FALSE, eval = FALSE}
...
```{r save transitionTypesGroups run, warning = FALSE, echo = FALSE}
# Create an R data frame that contains Transition Type Group names
transitionTypesGroups <- data.frame(TransitionTypeID = transitionTypes$Name,
                                    TransitionGroupID = transitionGroups$Name)
...
```{r save ages not run, warning = FALSE, eval = FALSE}
...
```{r save ages run, warning = FALSE, echo = FALSE}
# Define values for age reporting
ageFrequency <- 1
ageMax <- 101
ageGroups <- c(20, 40, 60, 80, 100)
...
```{r save runControl not run, warning = FALSE, eval = FALSE}
...
```{r save runControl run, warning = FALSE, echo = FALSE}
# Create an R data frame specifying to run the simulation for 7 realizations and 10 timesteps
runControl <- data.frame(MaximumIteration = 7,
                         MinimumTimestep = 0,
                         MaximumTimestep = 10,
...
```{r save dTransitions no run, warning = FALSE, eval = FALSE}
...
```{r save dTransitions run, warning = FALSE, echo = FALSE}
# Load  an empty Deterministic Transitions Datasheet to a new R data frame
dTransitions <- datasheet(myScenario, "stsim_DeterministicTransition", optional = T, empty = T)
...
```{r save pTransitions no run, warning = FALSE, eval = FALSE}
...
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Coniferous", 
                                          StateClassIDDest = "Deciduous", 
...
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Coniferous",
                                          ...
                                          Probability = 1, 
                                          AgeMin = 40))
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Deciduous",
                                          StateClassIDDest = "Deciduous", 
                                          TransitionTypeID = "Fire", 
                                          Probability = 0.002))
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Deciduous",
                                          ...
                                          Probability = 0.1, 
                                          AgeMin = 10))
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Mixed", 
                                          StateClassIDDest = "Deciduous", 
                                          TransitionTypeID = "Fire", 
                                          Probability = 0.005))
pTransitions <- addRow(pTransitions, data.frame(StateClassIDSource = "Mixed", 
...
```{r save pTransitions run, warning = FALSE, echo = FALSE}
# Load  an empty Probabilistic Transitions Datasheet to a new R data frame
pTransitions <- datasheet(myScenario, "stsim_Transition", optional = T, empty = T)
...
```{r plot initial conditions rasters, warning = FALSE, fig.align="center", fig.dim = c(5,5)}
...
```{r save ICSpatial no run, warning = FALSE, eval = FALSE}
...
```{r save ICSpatial run, warning = FALSE, echo = FALSE}
# Create an R list of the input raster layers
ICSpatial <- list(StratumFileName = stratumTif, 
                  StateClassFileName = sclassTif, 
                  AgeFileName = ageTif)
...
```{r check initial conditions rasters, fig.align="center", fig.dim = c(5,5), eval=FALSE}
# Load input raster layers from the `stsim_InitialConditionsSpatial` Datasheet.
rStratumTest <- datasheetRaster(myScenario, "stsim_InitialConditionsSpatial", "StratumFileName")
rSclassTest <- datasheetRaster(myScenario, "stsim_InitialConditionsSpatial", "StateClassFileName")
rAgeTest <- datasheetRaster(myScenario, "stsim_InitialConditionsSpatial", "AgeFileName")
...
```{r save ICNonSpatial not run, warning = FALSE, eval = FALSE}
...
```{r save ICNonSpatial run, warning = FALSE, echo = FALSE}
# Create non-spatial initial conditions data and add it to an R data frame
ICNonSpatial <- data.frame(TotalAmount = 100, 
                           NumCells = 100, 
                           CalcFromDist = F)
...
```{r view ICNonSpatial not run, warning = FALSE, eval = FALSE}
# View the updated Datasheet
datasheet(myScenario, "stsim_InitialConditionsNonSpatial")
```

```{r view ICNonSpatial run, warning = FALSE, echo = FALSE}
# View the updated Datasheet
datasheet(myScenario, "stsim_InitialConditionsNonSpatial")
```

```{r save ICNonSpatialDistribution not run, warning = FALSE, eval = FALSE}
...
```{r save ICNonSpatialDistribution run, warning = FALSE, echo = FALSE}
# Create non-spatial initial conditions distribution data and add it to an R data frame
ICNonSpatialDistribution <- data.frame(StratumID = "Entire Forest", 
                                       StateClassID = "Coniferous", 
                                       RelativeAmount = 1)
...
```{r view ICNonSpatialDistribution not run, warning = FALSE, eval = FALSE}
# View the updated Datasheet
datasheet(myScenario, "stsim_InitialConditionsNonSpatialDistribution")
```

```{r view ICNonSpatialDistribution run, warning = FALSE, echo = FALSE}
# View the updated Datasheet
datasheet(myScenario, "stsim_InitialConditionsNonSpatialDistribution")
```

**Transition Targets**: Define targets, in units of area, to be reached by the allocation procedure within SyncroSim.
```{r save transitionTarget not run, warning = FALSE, eval = FALSE}
...
```{r save transitionTarget run, warning = FALSE, echo = FALSE}
# Set the transition target for harvest to 0
saveDatasheet(myScenario, 
              data.frame(TransitionGroupID = "Harvest", 
                         Amount = 0),
...
```{r save outputOptionsSpatial not run, warning = FALSE, eval = FALSE}
...
```{r save outputOptionsSpatial run, warning = FALSE, echo = FALSE}
# Create output options for spatial model and add it to an R data frame
outputOptionsSpatial <- data.frame(
  RasterOutputSC = T, RasterOutputSCTimesteps = 1,
  RasterOutputTR = T, RasterOutputTRTimesteps = 1,
...
```{r save outputOptionsNonSpatial not run, warning = FALSE, eval = FALSE}
...
```{r save outputOptionsNonSpatial run, warning = FALSE, echo = FALSE}
# Create output options for non-spatial model and add it to an R data frame
outputOptionsNonSpatial <- data.frame(
  SummaryOutputSC = T, SummaryOutputSCTimesteps = 1,
  SummaryOutputTR = T, SummaryOutputTRTimesteps = 1
...
```{r save harvest transitionTarget not run, warning = FALSE, eval = FALSE}
...
```{r save harvest transitionTarget run, warning = FALSE, echo = FALSE}
# Create a copy of the no harvest scenario (i.e myScenario) and name it myScenarioHarvest
myScenarioHarvest <- scenario(myProject, 
                              scenario = "Harvest", 
                              sourceScenario = myScenario)
...
```{r view transitionTarget datasheets not run, warning = FALSE, eval = FALSE}
...
```{r view transitionTarget datasheets run, warning = FALSE, echo = FALSE}
# View the transition targets for the Harvest and No Harvest scenarios
datasheet(myProject, scenario = c("Harvest", "No Harvest"), 
          name = "stsim_TransitionTarget")
```
...
```{r run scenarios not run, warning = FALSE, eval = FALSE}
# Run both Scenarios
...
```{r run scenarios run, warning = FALSE, echo = FALSE}
# Run both Scenarios
myResultScenario <- run(myProject, scenario = c("Harvest", "No Harvest"), 
                     jobs = 7, summary = TRUE)
...
```{r view myRastersTimestep5 not run, warning = FALSE, eval = FALSE}
...
```{r view myRastersTimestep5 run, warning = FALSE, echo = FALSE}
# Retrieve the output State Class raster for the Harvest scenario at timestep 5
myRastersTimestep5 <- datasheetRaster(myProject, 
                                      scenario = resultIDHarvest, 
                                      "stsim_OutputSpatialState", 
...
```{r plot timestep 5 raster, fig.align = TRUE, fig.dim = c(5,5)}
