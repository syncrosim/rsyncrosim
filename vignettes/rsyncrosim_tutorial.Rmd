...
```
```{r, echo = FALSE}
out <- datasheet(myProject, summary = T, optional = F)
rmarkdown::paged_table(out)
```
...
```
```{r, echo = FALSE}
out <- subset(datasheet(myScenario, summary = T), scope == "scenario")
rmarkdown::paged_table(out)
```
...
```
```{r, echo = FALSE}
out <- datasheet(myProject, scenario = c("Harvest", "No Harvest"), 
                 name = "TransitionTarget")
rmarkdown::paged_table(out)
```
...
```
```{r, echo = FALSE}
out <- resultSummary <- run(myProject, scenario = c("Harvest", "No Harvest"), 
                            jobs = 7, summary = T)
rmarkdown::paged_table(out)
```
...
outputStratumState <- datasheet(myProject, 
                                scenario = c(resultIDNoHarvest, resultIDHarvest), 
                                name = "OutputStratumState")
rmarkdown::paged_table(outputStratumState)
```
