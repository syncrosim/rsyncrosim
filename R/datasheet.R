...
#'   \item {If ssimObject is a list of \code{\link{Scenario}} or \code{\link{Project}} objects (output from \code{\link{run}}, \code{\link{scenario}} or \code{\link{project}}): }{Adds ScenarioID/ProjectID column if appropriate.}
...
#' }
#'
#' @param ssimObject \strong{[SsimLibrary/Project/Scenario OR list of objects]}. Note that all objects in a list must be of the same type, and belong to the same library.
#' @param name \strong{[character OR character vector]}. Sheet name(s). If NULL, all datasheets in the ssimObject will be returned. Note that setting summary=FALSE and name=NULL pulls all datasheets, which is timeconsuming and not generally recommended.
#' @param project \strong{[character, numeric, OR vector of these]}. One or more \code{\link{Project}} names, ids or objects. Note that integer ids are slightly faster.
#' @param scenario \strong{[character, numeric, OR vector of these]}. One or more \code{\link{Scenario}} names, ids or objects. Note that integer ids are slightly faster.
#' @param summary \strong{[logical]}. If TRUE returns a dataframe of sheet names and other info. If FALSE returns dataframe or list of dataframes.
#' @param optional \strong{[logical]}. If summary=TRUE and optional=TRUE returns only scope, name and displayName. If summary=FALSE and optional=TRUE returns all of the datasheet's columns, including the optional columns. If summary=TRUE, optional=FALSE, returns only those columns that are mandatory and contain data (if empty=FALSE). Ignored if summary=FALSE, empty=FALSE and lookupsAsFactors=FALSE.
#' @param empty \strong{[logical]}. If TRUE returns empty dataframes for each datasheet. Ignored if summary=TRUE.
#' @param lookupsAsFactors \strong{[logical]}. If TRUE (default) dependencies returned as factors with allowed values (levels). Set FALSE to speed calculations. Ignored if summary=TRUE.
#' @param sqlStatement List returned by \code{\link{sqlStatement}}. SELECT and GROUP BY SQL statements passed to SQLite database. Ignored if summary=TRUE.
#' @param includeKey \strong{[logical]}. If TRUE include primary key in table.
#' @param forceElements \strong{[logical]}. If FALSE and name has a single element returns a dataframe; otherwise returns a list of dataframes. Ignored if summary=TRUE.
#' @param fastQuery \strong{[logical}].  If TRUE, the request is optimized for performance.  Ignored if combined with summary, empty, or \code{\link{sqlStatement}} flags.
#' 
#' @return 
