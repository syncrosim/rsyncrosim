...
#' console, or by directly querying the \code{\link[=SsimLibrary]{SsimLibrary-class}} database.
#'
#' @details
...
#' a SyncroSim \code{\link[=Scenario]{Scenario-class}}. \code{dataInherited} and \code{dataSource} columns
...
#'   \item If SsimObject is a list of \code{\link[=Scenario]{Scenario-class}} or \code{\link[=Project]{Project-class}}
#'          objects (output from \code{\link{run}}, \code{\link[=Scenario]{Scenario-class}} or
#'          \code{\link[=Project]{Project-class}}): Adds ScenarioId/ProjectId column if appropriate.
...
#' }
#'
#' @param ssimObject \code{\link[=SsimLibrary]{SsimLibrary-class}}, \code{\link[=Project]{Project-class}},
#'     or \code{\link[=Scenario]{Scenario-class}} object or list of objects.
...
#' @param project numeric \code{\link[=Project]{Project-class}} id
#' @param scenario numeric or numeric vector. One or more
#'     \code{\link[=Scenario]{Scenario-class}} ids
  ...
  function(
    ...
    if (is(xProjScn, "SsimLibrary")) {
      ...
      if (!is.null(sid) & is.null(pid)) {
        pid <- subset(
          xProjScn$scenarioSet,
          is.element(ScenarioId, sid)
        )$ProjectId
      }
    }
    ...
    if (!is.null(name)) {
      for (i in seq_along(name)) {
        ...
        if (!grepl("_", n, fixed = TRUE)) {
          stop(
            "The datasheet name requires a package prefix (e.g., 'stsim_RunControl')"
          )
        }
    ...
    # if summary, don't need project/scenario ids
    if (summary == TRUE) {
    ...
    if ((summary == TRUE) & !optional) {
      sumInfo <- subset(
        sumInfo,
        select = c("scope", "name", "displayName", "order")
      )
    ...
    # Add data info for scenario scope datasheets if sid is defined
    if (summary == TRUE) {
      ...
      for (i in seq(length.out = length(sid))) {
        cSid <- sid[i]
        tt <- command(
          list(list = NULL, datasources = NULL, lib = .filepath(x), sid = cSid),
          session = session(x)
        )

        hasDataInfo <- .dataframeFromSSim(
          tt,
          csv = FALSE,
          convertToLogical = c("data", "dataInherited")
        )
      ...
      sumInfo <- subset(
        sumInfo,
        select = c(prevNames, setdiff(names(sumInfo), prevNames))
      )
    ...
    for (kk in seq(length.out = length(allNames))) {
      if (summary == FALSE) {
        ...
        # Check if second part of filterColumn is not an integer
          ...
          if (fastQuery) {
            ...
            for (id in seq_along(sid)) {
              ...
              if (!identical(tt, "saved")) {
                ...
                # Rearrange on the spot, making sure this is robust for
                # Project-level datasheets as well
                IDColumnsForThisSheet <- IDColumns[IDColumns %in% names(sheet)]
                sheet <- sheet[, c(
                  IDColumnsForThisSheet,
              ...
              if (length(filterValue) > 1) {
                ...
                for (fv in filterValue) {
                  ...
                  if (!identical(ttLoop, "saved")) {
                    stop(
                      "Unable to export datasheet for filterValue '",
                      fv,
                      "': ",
                      ttLoop
                    )
                ...
                if (length(allSheets) == 0) {
                  stop(
                    paste0(
                      "None of the filterValue(s) ",
                      paste(filterValue, collapse = ", "),
      ...
      if (
        !is.null(filterColumn) &&
          (length(sid) > 1 || length(pid) > 1) &&
          exists("originalFilterColumn")
      ) {
        if (is.element(originalFilterColumn, names(sheet))) {
          ...
          # Apply the filter
          sheet <- sheet[
            sheet[[originalFilterColumn]] %in% originalFilterValue,
            ,
            drop = FALSE
          ]
        }
      ...
      if (empty | lookupsAsFactors | !returnInvisible) {
        ...
        if (!optional) {
          if (!empty) {
            sheetInfo$optional[
              is.element(sheetInfo$name, names(sheet)) &
                (sheetInfo$optional == "Yes")
            ] <- "Present"
          }
          sheetInfo <- subset(
            sheetInfo,
            is.element(optional, c("No", "Present"))
          )
        }
        ...
        if (lookupsAsFactors & !useConsole) {
          ...
          if (directQuery) {
            drv <- DBI::dbDriver("SQLite")
            con <- DBI::dbConnect(drv, .filepath(x))
            # console export can't handle multiple scenarios/projects; query database directly
          } else {
            tempFile <- paste0(.tempfilepath(x), "/", name, ".csv")
            args <- list(
              export = NULL,
            ...
            if (!identical(tt, "saved")) {
              stop(
                tt,
                "You might be asking for a datasheet at the project level but that datasheet has a scenario scope"
              )
            }
          }
        }
        ...
        for (i in seq(length.out = nrow(sheetInfo))) {
          ...
          if (
            (cRow$type %in% c("Integer", "Double", "Single")) &
              !(cRow$valType %in% c("DataSheet", "List"))
          ) {
          ...
          if (cRow$type == "Boolean") {
            if (length(setdiff(unique(sheet[[cRow$name]]), c(NA))) > 0) {
              ...
              sheet[[cRow$name]] <- as.logical(abs(as.numeric(sheet[[
                cRow$name
              ]])))
          ...
          if ((cRow$valType == "List") & lookupsAsFactors) {
            ...
            for (j in seq(length.out = length(opts))) {
              cLevels <- c(
                cLevels,
                strsplit(opts[j], ":", fixed = TRUE)[[1]][2]
              )
              cIDs <- as.numeric(c(
                cIDs,
                strsplit(opts[j], ":", fixed = TRUE)[[1]][1]
              ))
            }
          ...
          if (cRow$valType == "DataSheet") {
            if (lookupsAsFactors) {
              ...
              # console export can't handle multiple projects/scenarios; query database directly if necessary
              if (directQuery) {
              ...
              if (is.element("ProjectId", names(lookupSheet))) {
                if (identical(pid, NULL) & !identical(sid, NULL)) {
                  allScns <- scenario(x)
                  findPrjs <- allScns$ProjectId[is.element(
                    allScns$ScenarioId,
                    sid
                  )]
                } else {
              ...
              if (is.numeric(sheet[[cRow$name]])) {
                if (nrow(lookupSheet) > 0) {
                  ...
                  lookupMerge <- subset(
                    lookupSheet,
                    select = c(names(lookupSheet)[1], displayMem)
                  )
      ...
      if (is.element("ProjectId", names(sheet))) {
        if (length(pid) == 1) {
          ...
          if (nrow(sheet) > 0) {
            ...
            # Only keep the first two columns (ProjectId and Name)
            allProjects <- allProjects[, 1:2, drop = FALSE]
            # Merge by ProjectId to avoid duplicate columns
      ...
      if (is.element("ScenarioId", names(sheet))) {
        ...
        if (nrow(sheet) > 0 && returnScenarioInfo) {
          ...
          allScns <- subset(
            allScns,
            select = c(ScenarioId, ProjectId, Name, ParentId)
          )
          ...
          allScns <- subset(
            allScns,
            select = c(ScenarioId, ProjectId, Name, ParentId, ParentName)
          )
          ...
          if (is.element("ScenarioId", names(sheet))) {
            # if ScenarioId exists use as merge key
            sheet <- merge(allScns, sheet, by = "ScenarioId", all.y = TRUE)
          } else {
            # if ScenarioId was removed (single scenario case) don't remove it again
