...
setGeneric(
  ...
  function(
    ...
    standardGeneric("datasheetSpatRaster")
  }
)
...
setMethod(
  ...
  function(
    ...
    if (!is.null(scenario)) {
      warning(
        "scenario argument is ignored when ssimObject is a list of Scenarios"
      )
      scenario <- NULL
    }
...
setMethod(
  ...
  function(
    ...
    if (length(missingScns) > 0) {
      stop(
        "Scenarios not found in ssimObject: ",
        paste(missingScns, collapse = ",")
      )
    }
...
setMethod(
  ...
  function(
    ...
    cPropsAll$isRaster <- grepl(
      "isRaster^True",
      cPropsAll$properties,
      fixed = TRUE
    )
    ...
    while (tryCount <= 1) {
      ...
      if (!is.null(timestep) & is.element("Timestep", names(cMeta))) {
        ...
        if (length(missSteps) > 0) {
          warningMsg <- paste0(
            "Selected timesteps not available: ",
            paste(missSteps, collapse = ",")
          )
        }
      ...
      if (!is.null(iteration) & is.element("Iteration", names(cMeta))) {
        ...
        if (length(missSteps) > 0) {
          warningMsg <- paste0(
            warningMsg,
            " Selected iterations not available: ",
            paste(missSteps, collapse = ",")
          )
        }
    ...
    if ((length(nFiles) == 1) && (nrow(cMeta) > 1) && !is.null(cMeta$Band[1])) {
      ...
      cMeta$layerName <- paste0(
        strsplit(nFiles, ".", fixed = TRUE)[[1]][1],
        ".",
        cMeta$Band
      )
